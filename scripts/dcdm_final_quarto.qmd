...
```{r}
# Function to validate, update, complete and reorder row names in a file
check_files <- function(file_path) {
  ...
  if (length(missing_rows) > 0) {
    missing_data <- tibble(!!colnames(data)[1] := missing_rows)  # Create tibble with missing rows
    for (i in 2:ncol(data)) {
      missing_data[[colnames(data)[i]]] <- NA  # Fill other columns with NA
    }
    data <- bind_rows(data, missing_data)  # Combine original and missing rows
  }
    # Reorder rows based on expected_row_names; this is super important otherwise the collating function won't work
...
LOAD DATA LOCAL INFILE '/Users/aliciahobdell/Desktop/data_cleaning/DCDM_group3/data/collated/GroupingsTable.csv'
...
-   the diseases and procedures_table table contains **NO** NA values

-   the parameters table contain 143 rows with NAs corresponding to the non-IMPC IDs

-   the analyses table is completely messed up

```{r}
# Assuming analyses_table is your data frame

# Filter rows with NA values
rows_with_na <- parameters_table[apply(parameters_table, 1, function(row) any(is.na(row))), ]

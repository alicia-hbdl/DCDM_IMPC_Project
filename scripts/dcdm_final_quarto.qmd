...
OPTIONALLY ENCLOSED BY '"' 
IGNORE 1 LINES;
# Records: 177981  Deleted: 0  Skipped: 0  Warnings: 0
...
write.csv(genes_param_wide, "/Users/sama/Desktop/DCDM/data/cleaned/genes_phenotype_scores.csv", row.names = FALSE)

# Read the reshaped matrix
genes_param_wide <- read.csv("/Users/sama/Desktop/DCDM/data/cleaned/genes_phenotype_scores.csv")
...
write.csv(pca_input, "/Users/sama/Desktop/DCDM/scripts/phenotype_dashboard/pca_input.csv", row.names = FALSE)
...
library(pheatmap)
pheatmap(loadings, cluster_rows = TRUE, cluster_cols = TRUE)
...
write.csv(param_cluster_assignments, "/Users/sama/Desktop/DCDM/data/parameter_clusters.csv", row.names = FALSE)

write.csv(parameters_table, "/Users/sama/Desktop/DCDM/data/parameter_table.csv", row.names = FALSE)


# Load the cluster assignments and the parameter name mapping
param_clusters <- read.csv("/Users/sama/Desktop/DCDM/data/parameter_clusters.csv")
parameter_names <- read.csv("/Users/sama/Desktop/DCDM/data/parameter_table.csv")
...
# Group parameters by their cluster and assign meaningful names
...
# Create a list of parameter names under each cluster
# We will separate each parameter in a new row under the corresponding cluster
grouped_parameters_separated <- param_with_names %>%
  select(cluster, parameter_name) %>%  # Ensure only cluster and parameter_name are selected
  arrange(cluster)  # Sort by cluster for better visualization
...
write.csv(grouped_parameters_wide, "/Users/sama/Desktop/DCDM/data/grouped_parameters_wide.csv", row.names = FALSE)
...
# Tokenize the parameter names into keywords. 

tf_idf_by_cluster <- grouped_parameters_separated %>%
  unnest_tokens(word, parameter_name) %>%  # Split the parameter names into individual words
  count(cluster, word, sort = TRUE) %>%     # Count frequency of each word for each cluster
  bind_tf_idf(word, cluster, n) %>%          # Calculate TF-IDF (Term Frequency-Inverse Document Frequency)
  arrange(cluster, desc(tf_idf))             # Sort by TF-IDF within each cluster

# View the TF-IDF values for each cluster
print(tf_idf_by_cluster)

```

```{r}

# Assign grouping names and keywords

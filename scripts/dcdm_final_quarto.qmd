...
SELECT parameter_name FROM Parameters WHERE parameter_id IN (     SELECT parameter_id     FROM ParameterGroupings     WHERE group_id IN (         SELECT group_id         FROM Groupings         WHERE group_name = "brain"     ) );
```

Querying SQL Database for Gene x Parameter Tables:

We use the gene_accession_id and parameter_id instead of the gene names and parameter names due to different parameters having the same name and some parameters having no name.
...
Load Data into R and Reshape it into a wide format to create a matrix suitable for clustering genes based on phenotype scores.
...
#Query the database
#This ensures that the genes_and_param dataframe contains only one entry per unique gene_accession_id and parameter_id, with the average p_value.
query <- "
SELECT 
    gene_accession_id, 
    parameter_id, 
...
# Reshape data into a wide format (we want gene_accession_id as rows, parameter_ids as columns where pvalues for each parameter are shown in the cells)
genes_param_wide <- genes_and_param %>%
  pivot_wider(
    names_from = parameter_id, # Use parameter_id as column headers
    values_from = avg_rounded_pvalue, #Fill cells with rounded_pvalue
    values_fill = list(avg_rounded_pvalue = 0)  # Fill missing combinations with 0
  )
...
write.csv(genes_param_matrix, "genes_phenotype_scores.csv", row.names = FALSE)

...
```{r}
# Subset the table to include only the required columns in the correct order
procedure <- procedure[, c("impcParameterOrigId", "name", "description", "isMandatory")]
# Rename the columns to match the SQL table schema
colnames(procedure) <- c("procedure_id", "procedure_name", "procedure_description", "procedure_isMandatory")

# Save the table as CSV 
write.csv(procedure, file = paste0(rootDir, "/data/metadata/ProcedureTable.csv"), row.names = FALSE, quote = TRUE)
```
...
-- Path to tables 
LOAD DATA LOCAL INFILE '/Users/aliciahobdell/Desktop/data_cleaning/DCDM_group3/data/metadata/ProcedureTable.csv'

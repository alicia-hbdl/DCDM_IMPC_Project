...
parameters_table <- left_join(parameter, procedure, by = join_by(parameter_mapping)) %>% 
  ...
  mutate(parameter_group = case_when(
    ...
    str_detect(parameter_name, regex(
        ...
        ignore_case = TRUE)) ~ groupings_table$group_id[groupings_table$group_name == "eye"]))
...
CREATE TABLE ParameterGroupings (
    parameter_id VARCHAR(18),                        -- Foreign key to Parameters
    group_name VARCHAR(50),                                    -- Foreign key to Groupings by name
    PRIMARY KEY (parameter_id, group_id),            -- Composite key
    FOREIGN KEY (parameter_id) REFERENCES Parameters(parameter_id),
    FOREIGN KEY (group_name) REFERENCES Groupings(group_name)

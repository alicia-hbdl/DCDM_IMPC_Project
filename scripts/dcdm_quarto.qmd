...
-   Remove unnecessary quotes surrounding columns and add a comma instead.

-   Enclose the description column in double quotes to avoid commas in the description column to be misinterpreted as separators.

-   Remove the `line_number` variable as it is emppty, and equivalent to `procedure_id` duplicate the same unique information.

-   HTML symbols are incorrectly encoded, which can be corrected by HTML::Entities but will be ignored for simplicity.
...
# Line 2425-2427, 3872, 3873 had unnecessary enters removed manually 
# Remove leading and trailing quotes
sed -E 's/^"//; s/"$//; s/" "/,/g;' "${rootDir}/data/metadata/IMPC_parameter_description.txt" |
# Remove the line_number prefix
sed -E 's/^line_number,//' |
# Remove the corresponding line number
sed -E 's/^[0-9]{1,4},(.*)$/\1/' |
# Join lines where the next line doesn't start with a number
sed -E 'N; s/\n([^0-9])/\1/; P; D' |
sed -E 'N; s/\n([^0-9])/\1/; P; D' |
# Format selected fields with double quotes
sed -E 's/^([0-9]{4,5},)([^,]+),(.*),( IMPC_[A-Z]{3}_[0-9]{3}_[0-9]{3})$/\1"\2","\3",\4/' |
...
write.csv(procedures, file = paste0(rootDir, "/data/cleaned/Procedures.csv"), row.names = FALSE, quote = TRUE)
```
...
# Save the table as CSV 
write.csv(genes, file = paste0(rootDir, "/data/cleaned/Genes.csv"), row.names = FALSE, quote = TRUE)
```

```{sql}
#| eval: false
LOAD DATA LOCAL INFILE '/Users/aliciahobdell/Desktop/data_cleaning/DCDM_group3/data/cleaned/Genes.csv'
INTO TABLE Genes
FIELDS TERMINATED BY ',' 
OPTIONALLY ENCLOSED BY '"'
IGNORE 1 LINES;
...
parameters <- parameter_description %>%
  ...
  select(parameterId, name, description) %>%          # Select only the specific columns needed
  rename(parameter_id = parameterId,                  # Rename the columns
         parameter_name = name,                      
...
# Save the table as CSV 
write.csv(parameters, file = paste0(rootDir, "/data/cleaned/Parameters.csv"), row.names = FALSE, quote = TRUE)
```
...
```{sql}
LOAD DATA LOCAL INFILE '/Users/aliciahobdell/Desktop/data_cleaning/DCDM_group3/data/cleaned/Parameters.csv'

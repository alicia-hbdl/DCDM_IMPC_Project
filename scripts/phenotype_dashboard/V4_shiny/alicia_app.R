# Install packages
# install.packages(c("shiny", "ggplot2", "plotly", "dplyr", "cluster", "reshape2", "DBI", "RMySQL", "stringr", "factoextra", "umap", "dendextend", "ggdendro"))
  ...
  sidebarLayout(
    ...
    mainPanel(
      tabsetPanel(
        ...
        # Tab for Phenotype Scores for Knockout Mouse
        tabPanel(
        ...
        # Tab for Knockout Gene Scores for Phenotype
        tabPanel(
...
server <- function(input, output, session) {
  ...
  observe({
    gene_choices <- dbGetQuery(con, "SELECT DISTINCT gene_symbol FROM Genes ORDER BY gene_symbol ASC;") # Order alphabetically for ease-of-use
    updateSelectInput(session, "selected_mouse", choices = gene_choices$gene_symbol)
  })
  
  # Parameter grouping
  observe({
    groups <- dbGetQuery(con, "SELECT DISTINCT group_name FROM Groupings ORDER BY group_name ASC;") # Order alphabetically for ease-of-use
  ...
  observe({
    ...
    GROUP BY P.parameter_name ORDER BY P.parameter_name ASC;", input$selected_phenotype_group) # Order alphabetically for ease-of-use
  ...
  # Disease -- Where is this being used? 
  observe({
  ...
  observe({
    life_stages <- dbGetQuery(con, "SELECT DISTINCT mouse_life_stage FROM Analyses;") # Here, order needs to be hard coded some way for ease-of-use?
  ...
  # Render the conditional container for the plot
  output$plot_container <- renderUI({
    if (input$plot_type == "All Phenotypes") {
      # Scrollable container for All Phenotypes
      div(
        style = "overflow-x: auto; overflow-y: hidden; height: 600px;",
        plotlyOutput("mouse_phenotype_plot", width = "5000px", height = "100%")  # Extra-wide plot
      )
    } else {
      # Standard container for Top 25 Phenotypes
      plotlyOutput("mouse_phenotype_plot", height = "700px", width = "100%")
    }
  })
  ...
  output$mouse_phenotype_plot <- renderPlotly({
    ...
    output$no_data_message <- renderUI({
      ...
      if (is.null(data) || nrow(data) == 0) {
        ...
        tagList(
          h3("No data available for the selected mouse. Please adjust your filters.", 
    ...
    # Select rows with the 25 smallest p-values
    p <- if (input$plot_type == "Top 25 Phenotypes") {
      ggplot(data[order(data$p_value), ][1:25, ], aes(x = reorder(parameter_name, p_value), y = -log2(p_value), fill = Threshold)) +
      ...
      if (input$plot_type == "All Phenotypes") {
        p <- ggplot(data, aes(x = reorder(parameter_name, p_value), y = -log10(p_value), fill = Threshold, text = paste("p-value:", p_value, "<br>Phenotype:", parameter_name))) +
          ...
          theme_minimal() +
          theme(
            axis.text.x = element_text(angle = 30, hjust = 1, vjust = 0.5, size = 10, face = "bold"),
            axis.text.y = element_text(size = 10),
            axis.title.x = element_blank(),  # Remove default x-axis title
            axis.title.y = element_text(size = 12, face = "bold"),
            plot.title = element_text(size = 15, face = "bold", hjust = 0),    # Left-align plot title
            plot.subtitle = element_text(size = 12, hjust = 0),                # Left-align subtitle
            plot.margin = margin(t = 20, r = 20, b = 40, l = 100)              # Adjust margins for space
          ) +
          annotate(
            "text", x = -2, y = min(-log10(data$p_value)), label = "Knockout Mouse Phenotype", 
            angle = 90, hjust = 0.5, vjust = 1, size = 5, fontface = "bold"
          )
      }

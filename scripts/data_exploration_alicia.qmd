...
path = "/Users/aliciahobdell/Desktop/7BBG1003_data/DCDM_group3/"
```

## Combining the Raw Data in a Table

The dataset comprises thousands of CSV files, each containing information about mouse genetics, behavior, and phenotype analysis. The key fields include:

-   **`analysis_id`**: Unique alphanumeric identifier for each analysis.

-   **`pvalue`**: Statistical value indicating the likelihood of the observed result under the null hypothesis.

-   **`parameter_name`**: Name of the measured parameter or phenotype (e.g., a behavioral or physical trait).

-   **`gene_symbol`**: Shorthand notation for the associated gene.

-   **`mouse_strain`**: Genetic background of the mouse (e.g., specific inbred strain).

-   **`parameter_id`**: Unique identifier for the experimental parameter, possibly referencing a database standard.

-   **`gene_accession_id`**: Database-specific identifier for the gene (e.g., from Mouse Genome Informatics).

-   **`mouse_life_stage`**: Developmental stage of the mouse (e.g., "Early adult").
...
```{r}
SOP <- read.csv(paste0(path, "metadata/IMPC_sop.csv"))
SOP
```
...
```{r}
# Process the data table based on SOP constraints
process_data <- function(data_table, SOP) {
  ...
  for (field_name in field_names) {
    ...
    if (field_name == "mouse_strain") {
      ...
      field_specific_check <- which(!grepl("^[A-Z][a-z]*$", column))
    } 

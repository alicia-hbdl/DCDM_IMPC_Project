...
##### Reordering is unnecessary since information is merged/joined later based on matching fields, not indices {style="color: red"}
...
```{r}
# Function to validate, update, and complete row names in a file
check_files <- function(file_path) {
  # Validate column count; skip file if it doesn't have exactly 2 columns
  if (!validate_columns(file_path)) {
    message(paste("File", basename(file_path), "does not have exactly 2 columns. Skipping."))
    return(FALSE)
  }
  
  # Attempt to read the CSV file without headers
  analysis <- tryCatch(
    read_csv(file_path, col_names = FALSE, show_col_types = FALSE), 
    error = function(e) return(NULL)  # Handle file reading errors gracefully
  )
  ...
  # Normalize row names by converting them to lowercase
  analysis[[1]] <- tolower(analysis[[1]])
  
  # Identify missing rows from `expected_row_names` and add them with NA values
  missing_rows <- setdiff(expected_row_names, analysis[[1]])
  if (length(missing_rows) > 0) {
    missing_data <- tibble(!!colnames(analysis)[1] := missing_rows)  # Create tibble with missing rows
    for (i in 2:ncol(analysis)) {
      missing_data[[colnames(analysis)[i]]] <- NA  # Fill other columns with NA
    }
    analysis <- bind_rows(analysis, missing_data)  # Combine original and missing rows
  }
  
  # Save the updated file without headers to the updated data directory
...
```{r}
# Define a function to clean and process a single file
combine_files <- function(file_path) {
  read_csv(file_path, show_col_types = FALSE) %>%
    t() %>%                           # Transpose the data: fields become columns, values form rows
    as.data.frame() %>%               # Convert the transposed data into a data frame
    setNames(.[1, ]) %>%              # Set the first row as column names
    slice(-1) %>%                     # Remove the first row (now column names)
    mutate(analysis_id = rownames(.), .before = 1)  # Add `analysis_id` as the first column
}

# Process all CSV files in the updated data directory and combine into a single table
analysis <- dir_ls(path = updated_data_dir, glob = "*.csv") %>%
  map_dfr(combine_files) %>%            # Apply `combine_files` to each file and combine results
  write_csv(paste0(rootDir, "data/raw_data/analysis_table.txt"))  # Save the combined table as a CSV

# Display the first 6 rows of the combined analysis table
head(analysis)
```
...
# Cap pvalues greater than 1 to 1
analysis <- analysis %>%
  mutate(pvalue = ifelse(pvalue > 1, 1, pvalue))  # Fix values >1 based on observed issues
...
log_message(if (nrow(invalid_pvalues) == 0) {
  "All pvalue values are valid."  # Log success if no issues
} else {
...
This step subsets and transforms the cleaned data to align with the agreed-upon database schema. Each section focuses on extracting specific fields, applying necessary transformations, and saving the results as CSV files ready for upload into the database.

The cleaned input tables are loaded to avoid rerunning the entire script.
...
The `procedures_table` is created by selecting key fields from the `procedure` table: `procedure_id`, `procedure_name`, `procedure_description`, and `is_mandatory`. The `is_mandatory` field is converted to integers (0 or 1) to meet SQL requirements for boolean fields. Duplicate rows are removed to ensure only unique entries remain. The table is saved as `Procedures.csv`.
...
The `genes_table` is built by combining `gene_accession_id` and `gene_symbol` from the `analysis` and `disease` tables. Gene symbols are converted to title case, and specific gene accessions are mapped to predefined symbols for consistency. Unique entries are retained, and the final table is saved as `Genes.csv`.
...
The `analyses_table` is created by extracting relevant fields from the `analysis` table: `analysis_id`, `gene_accession_id`, `mouse_strain`, `mouse_life_stage`, `parameter_id`, and `pvalue`. Duplicate rows are removed to ensure clean data, and the table is saved as `Analyses.csv`.
...
The `parameters_table` is generated by joining the `parameter` and `procedure` tables using a mapping column (`parameter_mapping`). It ensures that all `parameter_id` values present in the `analyses_table` but missing from the `parameter` table are added. The resulting table includes fields like `parameter_id`, `parameter_name`, `parameter_description`, and `procedure_id`, and is saved as `Parameters.csv`.
...
The `diseases_table` is created by selecting `disease_id` and `disease_term` from the `disease` table, ensuring only unique rows are included. The processed table is saved as `Diseases.csv`.
...
The `phenodigm_scores` table is created by selecting `disease_id`, `gene_accession_id`, and `phenodigm_score` from the `disease` table. This table is saved as `PhenodigmScores.csv` for further analysis.
...
The goal is to group parameters based on their naming or similarity in phenotype tests. This helps reduce the parameter space and organize them into meaningful categories (e.g., `weight`, `images`, `brain`, and others). These groupings are essential for downstream analysis and database design. The collaborator requires at least three additional parameter groups beyond the initial ones provided (`weight`, `images`, `brain`).

#### Define Group Patterns

The `group_patterns` table is created to define group names (e.g., "weight", "images", "brain") and associated regular expression patterns. These patterns are used to match parameter names that belong to each group. For example, the "weight" group includes terms such as "weight", "density", and "BMI", while the "brain" group matches terms like "cortex", "hippo", and "neural". This table acts as the basis for identifying and categorizing parameters.
...
A new `groupings_table` is generated by dynamically assigning a unique ascending ID (`group_id`) to each group name from the `group_patterns` table. Only the `group_id` and `group_name` columns are retained, and the resulting table is saved as a CSV file named `GroupingsTable.csv` in the collated data directory. This table serves as a reference for linking parameters to their respective groups in the database.

```{r}
# Create a dynamic group table with ascending IDs and group names from the lookup table
groupings_table <- group_patterns %>%
  mutate(group_id = row_number()) %>%   # Add ascending IDs for each group
  select(group_id, group_name)         # Keep only group_id and group_name columns
...
The `parameter` table is processed to associate each `parameter_id` with one or more groups based on the regex patterns defined in `group_patterns`. The `parameter_name` column is matched against the regex patterns, and matching group names are added as a new column. If a parameter matches multiple groups, the data is expanded to create a separate row for each match. The resulting table is then joined with the `groupings_table` to add the corresponding `group_id` for each group, and duplicates are removed. The final table, `groupXparameter_table`, contains only `parameter_id` and `group_id` columns and is saved as a CSV file named `ParameterGroupings.csv` in the collated data directory.
...
This code organizes parameters into groups to reduce the parameter space, aligning with the collaborator's request to group similar phenotype tests and parameter names. By dynamically creating groupings and mapping parameters to these groups, the code enables efficient data categorization for further database integration and analysis.
...
WHERE Parameters.parameter_name IN ("Fusion of vertebrae", "Hindbrain morphology", "Limb morphology") 
ORDER BY Analyses.p_value ASC;
```

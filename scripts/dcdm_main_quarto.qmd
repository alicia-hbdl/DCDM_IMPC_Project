...
data_dir <- paste0(rootDir, "data/raw_data") # Input raw data directory
output_dir <- paste0(rootDir, "data/updated_data") # Ouptut cleaned data directory
...
expected_row_names <- tolower(read_csv(SOP_path, show_col_types = FALSE)$dataField)
...
process_file <- function(file_path) {
  ...
  # If data is NULL (file could not be read), return FALSE
  if (is.null(data)) return(FALSE)
  
  # Convert the first column to lowercase for consistency
  ...
  # Reorder rows to match the order of 'expected_row_names'
...
# Create a summary message with the total and successfully updated files
...
```{r}
data_dir <- "updated_data"  # Input directory containing the cleaned files

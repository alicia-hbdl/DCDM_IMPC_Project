...
```{r}
# Prepare the genes table by subsetting, ensuring unique rows, converting gene symbols to title case, and filling missing data
genes <- analysis %>%
  select(gene_accession_id, gene_symbol) %>%   # Select relevant columns
  distinct() %>%         # Remove duplicates
  bind_rows(              # Add missing gene_accession_ids from `disease`
    disease %>%
      select(gene_accession_id) %>%
      filter(!gene_accession_id %in% genes$gene_accession_id) %>% # Identify missing IDs
      mutate(gene_symbol = NA)      # Assign NA for missing gene symbols
  ) %>%
  distinct()                  # Ensure the final table is unique

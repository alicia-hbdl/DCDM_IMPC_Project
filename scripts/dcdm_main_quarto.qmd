...
#### This code duplicates the logic used to modify the files, making it somewhat redundant and unable to detect field name issues. To streamline the process, consider enhancing the first code to include the additional checks (e.g., verifying that the file has exactly two columns). {style="color: red"}
...
To check the differences between the raw and cleaned files:
...
-   `<`: Lines present in the `raw_data` file but missing in the `updated_data` file.

-   `>`: Lines present in the `updated_data` file but missing in the `raw_data` file.

-   `c`: Lines that differ between the two files, shown as changes.
...
Each row in the `IMPC_procedure.txt` file contains: a line number, procedure name, description, a mandatory boolean field (`TRUE/FALSE`), and the original IMPC ID.

**Key Issues:**

-   **Unnecessary double quotes**: These make parsing unreliable.

-   **Misinterpreted commas**: Commas in the description field require proper quoting.

-   **Incorrectly encoded HTML symbols**: Symbols like `&amp;` and `&lt;` need decoding.

-   **Field duplication**: `line_number` and `procedureId` contain redundant information.

To address these issues, the process involves:

-   **Formatting Cleanup**: Ensuring the raw text is suitable for conversion into a proper CSV file.

-   **Data Cleanup**: Ensuring consistency and clarity within the table.

```{r}
cleaned_procedure_path <- paste0(rootDir, "data/cleaned/cleaned_IMPC_procedure.txt")

# Read, process, and clean the raw procedure file
procedure <- read_lines(paste0(rootDir, "data/metadata/IMPC_procedure.txt")) %>%
  ...
  str_remove_all('^"|"$') %>%
  # Wrap descriptions with commas in quotes for proper CSV formatting
  str_replace('^([0-9]+,[^,]+,)(.*)(, (TRUE|FALSE), [0-9]{5})$', '\\1"\\2"\\3') %>%
  # Remove the redundant "line_number" field
  str_remove('^line_number,') %>%
  # Decode HTML entities to ensure readable and accurate text
  str_replace_all("&amp;", "&") %>%
  str_replace_all("&nbsp;", " ")

# Save the cleaned lines to a new file
write_lines(procedure, cleaned_procedure_path)

# Read the cleaned file as a CSV and clean data values
procedure <- read_csv(
  paste0(rootDir, "data/cleaned/cleaned_IMPC_procedure.txt"),
  col_names = TRUE
) %>%
  # Trim whitespace, convert to lowercase, and replace empty strings or "NA" with NA
  mutate(across(everything(), ~ .x %>% tolower() %>% str_trim() %>% na_if("") %>% na_if("na"))) %>%
  ...
  group_by(procedure_name, is_mandatory) %>%
  summarise(
    # Assign the same unique ID to all rows within the group
    procedure_id = cur_group_id(),
    # Store all parameter_mapping values as a list
    parameter_mapping = list(parameter_mapping),
    procedure_description = list(unique(procedure_description)),
    # Select the longest procedure_description
    # procedure_description = procedure_description[which.max(str_length(procedure_description))],
...
# Save the cleaned lines to a new file
write_csv(procedure, cleaned_procedure_path)

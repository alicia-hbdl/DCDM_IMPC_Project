...
rm(log_file_mod, log_file_valid, raw_data_dir, updated_data_dir, valid_files, total_files)
```
...
```{r}
procedure_path <- paste0(rootDir, "data/metadata/IMPC_procedure.txt")
reformatted_procedure_path <- paste0(rootDir, "data/cleaned/reformatted_IMPC_procedure.txt")

# Reformat for proper table loading
procedure <- read_lines(procedure_path) %>% 
...
# Save the reformatted lines to a new file
write_lines(procedure, reformatted_procedure_path)
```
...
procedure <- read_csv(reformatted_procedure_path, col_names = TRUE) %>%
...
cleaned_procedure_path <- paste0(rootDir, "data/cleaned/cleaned_IMPC_procedure.csv")
# Save the cleaned data to a new CSV file
write_csv(procedure, cleaned_procedure_path)
file.remove(reformatted_procedure_path)
```

...
process_file <- function(file_path) {
  ...
  write.table(data, file.path(updated_data_dir, basename(file_path)), quote = FALSE)
...
```{bash}
diff -r /path/to/raw_directory /path/to/cleaned_directory
```
...
all_files <- list.files(paste0(rootDir, "data/updated_data"), pattern = "\\.csv$", full.names = TRUE)
...
procedure_outputFile <- paste0(rootDir, "/data/cleaned/cleaned_IMPC_procedure.txt")

# Read the file
procedure <- readLines(procedure_path, encoding = "UTF-8")
...
writeLines(procedure, procedure_outputFile)
```

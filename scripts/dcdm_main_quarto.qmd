...
A second step cleans the table to remove duplicate rows, implement consistent column names, and ensure that all values are in the correct format.
...
Some procedures, such as "MicroCT E14.5-E15.5" and "MicroCT E14.5-E15.5 Analysis," were merged as they referred to the same test. Similarly, procedures with the same name but slightly different or missing descriptions (e.g., "Experimental Design," "Housing and Husbandry," "Electrocardiogram (ECG)," and "Viability Primary Screen") were standardized: missing descriptions were filled using existing values, and conflicts were resolved by selecting the most appropriate description.
...
procedure <- read_csv(temporary_path, show_col_types = FALSE, col_names = TRUE) %>%
  ...
  # Standardize conflicting names and descriptions for rows referring to the same observation
...
parameter <- read_csv(temporary_path, show_col_types = FALSE) %>%
  ...
  distinct(parameter_name, parameter_description, parameter_id, .keep_all = TRUE)  # Reduces rows from 5325 to 3600
...
-   The gene_accession_id column in the SOP is described as a 9-11 character string starting with MGI: and followed by a unique numeric ID, with values being unique.

-   In this data-set, duplicates in gene_accession_ids are permissible because the same gene can be associated with multiple diseases.
...
Step 4: Saving and Loading the Final Data

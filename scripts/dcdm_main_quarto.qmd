...
write_csv(groupings_table, paste0(rootDir, "/data/collated/GroupingsTable.csv"))
```
...
# Save the table as CSV 
write_csv(procedures_table, paste0(rootDir, "/data/collated/Procedures.csv"))
```
...
write_csv(genes_table, paste0(rootDir, "/data/collated/Genes.csv"))
```
...
parameters_table <- left_join(parameter, procedure, by = join_by(parameter_mapping)) %>% 
  ...
  mutate(parameter_group = case_when(
    str_detect(parameter_name, regex(
      ...
      ignore_case = TRUE)) ~ groupings_table$group_id[groupings_table$group_name == "weight"],
        str_detect(parameter_name, regex(
      ...
      ignore_case = TRUE)) ~ groupings_table$group_id[groupings_table$group_name == "images"],
    str_detect(parameter_name, regex(
        ...
        ignore_case = TRUE)) ~ groupings_table$group_id[groupings_table$group_name == "brain"],
    str_detect(parameter_name, regex(
        "heart|cardiac|vascular|blood|vein|aort|arter|HR|stroke|ejection|QT|circulation|pulse|pressure|rate|systolic|diastolic|respiration|ECG|cardio|valve",       ignore_case = TRUE)) ~ groupings_table$group_id[groupings_table$group_name == "cardiovascular"]))

write_csv(parameters_table, paste0(rootDir, "/data/collated/Parameters.csv")) 
```
...
write_csv(diseases_table, paste0(rootDir, "/data/collated/Diseases.csv"))
```
...
write_csv(analyses_table, paste0(rootDir, "/data/collated/Analyses.csv"))
```
...
CREATE TABLE GroupingsTable (
    group_id INT PRIMARY KEY ,   -- Automatically increments for each new row
    group_name VARCHAR(50)                     -- Name of the parameter group
) COMMENT = 'Defines groups of parameters_table. Linked to Parameters via a one-to-many join.';
...
CREATE TABLE Parameters (
    ...
    group_id INT,                              -- Foreign key to GroupingsTable
    FOREIGN KEY (procedure_id) REFERENCES Procedures(procedure_id),
    FOREIGN KEY (group_id) REFERENCES GroupingsTable(group_id)
) COMMENT = 'Stores parameter metadata. References Procedures (many-to-one), links to GroupingsTable (many-to-one) and referenced by Analyses (one-to-many).';

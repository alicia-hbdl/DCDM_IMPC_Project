...
log_file <- paste0(rootDir, "scripts/row_name_cleaning.log") # Log file for validation & modifications
...
process_file <- function(file_path) {
  ...
  write_csv(data, updated_data_dir, col_names = FALSE)
...
process_file <- function(file_path) {
  read_csv(file_path, header = FALSE, col.names = c("field", "value")) %>%
...
write_csv(analysis_table, paste0(rootDir, "data/cleaned/analysis_table.txt"))
...
data <- read_csv(paste0(rootDir, "/data/analysis_table.txt"))
...
expected_data_types <- read_csv(SOP_path, show_col_types = FALSE)$dataType %>% recode("String" = "character", "Float" = "numeric") # Convert datatypes to those recognised by R
...
```{r}
# Load procedure table temporarily, normally it is already in the env. 
analysis <- read_csv(paste0(rootDir, "data/cleaned/cleaned_IMPC_procedure.csv")) 
...
```{r}
# Load analysis table temporarily, normally it is already in the env. 
analysis <- read_csv(paste0(rootDir, "data/cleaned/cleaned_analysis_table.txt")) 

genes <- analysis %>% select(gene_accession_id, gene_symbol) %>% # Select desired columns
  mutate(
    gene_symbol = str_to_title(gene_symbol))%>% # Convert to title case (optional, this should have been done in a previous step)
# Ensure all genes from analysis are in the genes table 
  bind_rows(disease %>% 
      select(gene_accession_id) %>%
      filter(!gene_accession_id %in% analysis$gene_accession_id)) %>%
  # Add symbol to genes with NA symbols based on MGI
  mutate(
    gene_symbol = case_when(
      ...
      TRUE ~ gene_symbol)) %>% 
  distinct()                  # Ensure the final table has unique rows

View(genes)

# Save the table as CSV 
write_csv(genes, file = paste0(rootDir, "/data/collated/Genes.csv"))
rm(genes)
```
...
parameter <- read_csv(paste0(rootDir, "data/cleaned/cleaned_IMPC_parameter_description.csv"))
...
write_csv(parameters, file = paste0(rootDir, "/data/collated/Parameters.csv")) 
```

# Diseases

```{r}
# Select the subset of columns required for the disease table
diseases <- disease %>%
...
```{r}
# Load analysis table 
analysis <- read_delim(paste0(rootDir, "data/cleaned/cleaned_analysis_table.txt"), delim = ",")

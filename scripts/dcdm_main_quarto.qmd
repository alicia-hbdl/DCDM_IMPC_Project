...
write_lines(procedure, paste0(rootDir, "data/cleaned/cleaned_IMPC_procedure.txt")
```

##### **Data Cleanup**: Ensuring consistency and clarity within the table.

##### Once the code exists for the SOP of analysis, the same constraints can be applied here.  {style="color: red"}
...
procedure <- read_csv(
  ...
  # Trim whitespace and replace empty strings or "NA" with NA
  mutate(across(everything(), ~ .x %>% str_trim() %>% na_if("") %>% na_if("NA"))) %>%
  ...
  rename(
    ...
    parameter_id = impcParameterOrigId
  )

# Preview the first few rows of the cleaned table
head(procedure)
```

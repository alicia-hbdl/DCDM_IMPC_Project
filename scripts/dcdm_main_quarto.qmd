...
```{r}
# Load the SOP file and normalize row names (remove white spaces and conver to lowercase)
expected_row_names <- trimws(tolower(read_csv(SOP_path, show_col_types = FALSE)$dataField))
...
```{r}
# Function to correct row names
process_file <- function(file_path) {
  # Attempt to read the CSV file specified by file_path
  data <- tryCatch(
    # col_names = FALSE treats all rows as data, as the files lack headers
    # suppress console messages about column type detection
    read_csv(file_path, col_names = FALSE, show_col_types = FALSE),
    # On error, log the issue and set 'data' to NULL
    error = function(e) {
      message(paste("Error reading file:", file_path, "-", e))
      return(NULL) # Exit with NULL if file reading fails
    }
  )
  # If the file could not be read, return FALSE
  if (is.null(data)) return(FALSE)
  
  # Convert the fields (first column) to lowercase and remove leading/trailing spaces for consistency
  data[[1]] <- trimws(tolower(data[[1]]))
  
  # Identify rows that are expected but missing in the current data
  missing_rows <- setdiff(expected_row_names, data[[1]])
  
  # Add missing rows to the data, assigning NA to the second column
  if (length(missing_rows) > 0) {
    missing_data <- tibble(
      !!colnames(data)[1] := missing_rows, # Use the name of the first column for the row names
      !!colnames(data)[2] := NA            # Assign NA to the second column for missing rows
    )
    data <- bind_rows(data, missing_data)
  }
  
  # Reorder rows to match 'expected_row_names' (optional, as join/merge functions rely on matching field names rather than the order of rows)
  # data <- data %>% arrange(match(data[[1]], expected_row_names))

  # Save the updated data to the output directory
  write.table(data, file = file.path(updated_data_dir, basename(file_path)), sep = ",", row.names = FALSE, col.names = FALSE)
...
# Process each file and store success/failure results
validation_results <- sapply(files, process_file)

# Create a summary message with the total and successfully processed files
summary_message <- sprintf(
  "Validation Summary:\nTotal files checked: %d\nTotal successfully processed files: %d\n",
  length(validation_results), sum(validation_results)
)

# Write the summary to the log file and display it in the console
cat(summary_message, file = log_file_mod)
...
#### This code duplicates the logic used to modify the files, making it somewhat redundant and unable to detect field name issues. To streamline the process, consider enhancing the first code to include the additional checks (e.g., verifying that the file has exactly two columns); + we will be running the SOP on the cleaned table eventually, so this was probably really helpful when developing the code, but isn't very interesting for the final script  {style="color: red"}
...
```{r}
# Function to validate a file against the SOP layout and column requirements
validate_file <- function(file_path, expected_row_names) {
  ...
  # If the file couldn't be read, return an invalid result
  ...
  # Normalize row names and compare with expected SOP row names
  row_names <- tolower(data[[1]])
  missing_rows <- setdiff(expected_row_names, row_names) # Rows missing from the file
  extra_rows <- setdiff(row_names, expected_row_names) # Rows in the file not in the SOP
  
  # Return invalid result if row name mismatches are found
  ...
  # Return valid result if all checks pass
...
# Validate all files in the updated data directory
files <- list.files(updated_data_dir, pattern = "\\.csv$", full.names = TRUE)
validation_results <- lapply(files, function(file_path) {
  # Validate each file and log results
  result <- validate_file(file_path, expected_row_names)
  message <- paste(basename(file_path), ":", result$message, "\n")
  cat(message, file = log_file_valid, append = TRUE) # Append results to the log file 
  return(result$valid)
})

# Summarize and log the validation results
valid_files <- sum(unlist(validation_results)) # Count valid files
total_files <- length(validation_results) # Total files checked

# Create a summary message with the total and valid files
summary_message <- sprintf(
  "Validation Summary:\nTotal files checked: %d\nTotal valid files: %d\n",
...
rm(temporary_path, process_file)
```

### IMPC_parameter_description.txt (Sama) {style="color: green"}
...
parameter <- read_lines(paste0(rootDir, "data/metadata/IMPC_parameter_description.txt")) %>%
  ...
  unlist() %>%                                # Flatten into a character vector
 # str_trim() %>%                              # Trim whitespace
  str_replace_all('\\(([^)]*)\\)', function(x) str_replace_all(x, ',', ';')) %>% # Replace commas in parentheses
  str_replace_all(                            # Match fields with regex and enclose them with quotes
    '^([1-9][0-9]{0,3}),([0-9]{4,5}), ?([^,]+), (.*), ?(IMPC_[A-Z]{3}_[0-9]{3}_[0-9]{3})$', 
    '"\\2","\\3","\\4","\\5"'                 # Space after comma would cause error with read_csv
  ) %>%
  write_lines(paste0(rootDir, "data/cleaned/formatted_IMPC_parameter_description.txt")) # Save to file

parameter <- read_csv(
  paste0(rootDir, "data/cleaned/formatted_IMPC_parameter_description.txt"),
  quote = ""   # Use this only if quotes are problematic; otherwise, `read_csv` handles them automatically
) %>%
  filter(!str_detect(parameter_id, '^IMPC_[A-Z]{3}_[0-9]{3}_[0-9]{3}$'))

```

The initial metadata file contains five intended columns: `line_number`, `impcParameterOrigId`, `name`, `description`, and `parameterId`. However, the entire dataset is being read as a single string under a single column labeled `x`, with the first two columns merged and improperly delimited. Upon opening the file in a delimited format, observations reveal potential redundancy between the `name` and `description` fields, as they often describe the same parameter in different styles. All data is treated as characters, including numeric values, due to the absence of properly specified column names. Additionally, 41 issues were identified where commas within parentheses caused abnormal splits between columns, which can be resolved by replacing these commas with semicolons. By using the `separate()` function with `extra = "merge"` to combine excess fields and `fill = "right"` to handle missing values, the single-column dataset can be split correctly. Finally, column names were standardized to match the SOP.
...
field_names <- colnames(parameter)
print(field_names)
...
fix_parameter_id <- function(parameter) {
  parameter <- parameter %>%
    mutate(
...
parameter <- fix_parameter_id(parameter)
```
...
write.csv(parameter, file = output_file_parameter, row.names = FALSE)

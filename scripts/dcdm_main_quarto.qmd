...
rm(log_file_mod, log_file_valid, raw_data_dir, valid_files, total_files, validation_results, files, summary_message, expected_row_names)
```
...
write_csv(genes, paste0(rootDir, "/data/collated/Genes.csv"))
```

# Parameters

```{r}
parameters <- left_join(parameter, procedure, by = join_by(parameter_mapping)) %>% 
  distinct(parameter_id, parameter_name, parameter_description, procedure_id)

write_csv(parameters, paste0(rootDir, "/data/collated/Parameters.csv")) 
```

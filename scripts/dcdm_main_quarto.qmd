...
analysis <- dir_ls(path = updated_data_dir, glob = "*.csv") %>%
  map_dfr(clean_files) %>%
  write_csv(paste0(rootDir, "data/analysis_table.txt"))
...
analysis <- read_csv(paste0(rootDir, "data/analysis_table.txt"), show_col_types = FALSE)
...
parameters_table <- left_join(parameter, procedure, by = join_by(parameter_mapping)) %>% 
  ...
  mutate(parameter_group = case_when(
    str_detect(parameter_name, regex(
      ...
      ignore_case = TRUE)) ~ groupings$group_id[groupings$group_name == "weight"],
        str_detect(parameter_name, regex(
      ...
      ignore_case = TRUE)) ~ groupings$group_id[groupings$group_name == "images"],
    str_detect(parameter_name, regex(
        ...
        ignore_case = TRUE)) ~ groupings$group_id[groupings$group_name == "brain"],
    str_detect(parameter_name, regex(
        "heart|cardiac|vascular|blood|vein|aort|arter|HR|stroke|ejection|QT|circulation|pulse|pressure|rate|systolic|diastolic|respiration|ECG|cardio|valve",       ignore_case = TRUE)) ~ groupings$group_id[groupings$group_name == "cardiovascular"]))
...
) COMMENT = 'Defines groups of parameters_table. Linked to Parameters via a many-to-many join.';
...
) COMMENT = 'Stores parameter metadata. References Procedures (many-to-one), links to ParameterGroupings (many-to-many) and referenced by Analyses (one-to-many).';
